...
<div class="text-center p-5  text-teal-800 ">
    ...
    {{-- ha falta de arreglar el old y los errores --}}
    ...
    <form action="{{route('partits.create_partits')}}" method="POST" class="max-w-3xl p-8 m-auto">
        ...
        {{-- <label>Estat del partit:<br>
            @error('entitat')            
            <small style="color:red">*{{$message}}</small>
            <br>
            @enderror
        </label>
        <select name="estat" class="mb-6 bg-gray-100 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-gray-400 dark:focus:ring-blue-500 dark:focus:border-blue-500" value="{{old('data')}}">
            
            <option value="Pendent">Pendent</option>
            <option value="Juagant">Juagant</option>
