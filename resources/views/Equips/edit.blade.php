...
<form action="{{route('equips.update', $equips)}}" method="POST" class="max-w-3xl p-8 m-auto">
    ...
    <select name="entitat" class="mb-6 bg-gray-100 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-gray-400 dark:focus:ring-blue-500 dark:focus:border-blue-500" value="{{old('entitat', )}}">
        @foreach($entitats as $entitat)
        <option value="{{$entitat->id}}">{{$entitat->nom_club}}</option>
        @endforeach
    </select>
    ...
    <div class="flex justify-around">
        ...
        <a class="px-4 py-2 rounded-md bg-teal-500 text-sky-100 hover:bg-teal-800" href="{{route('equips.index')}}">Cancelar</a>
       </div>
</form>
@endsection
